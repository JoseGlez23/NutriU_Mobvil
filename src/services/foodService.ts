...
const logFoodServiceWarning = (message: string, error?: any) => {
  if (__DEV__) {
...
export const foodService = {
  ...
  registerFood: async (
    ...
    if (error) {
      logFoodServiceWarning("Error al registrar alimento", error);
    }
  ...
  getFoodHistory: async (
    ...
    if (error) {
      logFoodServiceWarning("Error al obtener historial", error);
    }
  ...
  getAvailableFoods: async () => {
    ...
    if (error) {
      logFoodServiceWarning("Error al obtener alimentos", error);
    }
